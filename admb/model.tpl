...
PARAMETER_SECTION
  ...
  // |                       as a 'dev' vector, it sums to zero
  // |
  // | - Fmort            <- F * selectivity
  // |
  ...
  // | - vector for selectivity selectivity
     ...
     init_bounded_dev_vector  log_F_devs(styr,endyr,-5.,5.,ph_F);
...
FINAL_SECTION
  ...
  // | derived quantities for each bootstrap iteration
    ...
    for(ii = 83; ii<=118; ii++)
      {
        log_F_devs(ii - 83+styr) = bootN(ii);   // F deviations
      }


        fish_sel_a50 = bootN(119);              // Age at 50% fishery selectivity


    for(ii = 120; ii<=128; ii++)
      {
        mF(ii-119) = bootN(ii);                // F levels for projected N
      }
